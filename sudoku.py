'''to perform all tests: python validator.py --stdio zad4 python sudoku.py
Besides that, to run some commands from validator.py there is a need to add --stdio flag into it'''
...
def boxx(i):
    return [V(j, k) for j in range((i//3)*3, (i//3)*3+3) for k in range((i % 3)*3, (i % 3)*3+3)]


def box():
    return [all_different(boxx(i)) for i in range(9)]
